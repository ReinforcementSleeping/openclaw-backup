...
import path__default, { dirname, resolve } from 'node:path';
...
function resolveStateDirFromEnv(env = process.env) {
    ...
    if (env.VITEST || env.NODE_ENV === "test") {
        return path__default.join(os__default.tmpdir(), ["openclaw-vitest", String(process.pid)].join("-"));
    }
    return path__default.join(os__default.homedir(), ".openclaw");
}
function resolveReqIdFilePath(accountId) {
    const safe = accountId.replace(/[^a-zA-Z0-9_-]/g, "_");
    return path__default.join(resolveStateDirFromEnv(), "wecom", `reqid-map-${safe}.json`);
}
// ============================================================================
// 核心实现
// ============================================================================
